...
struct OnboardingView: View {
    ...
    private func startDataImport() {
        ...
        Task {
            do {
                // Initial progress - represents preparing for import (20%)
                for i in 1...5 {  // Reduced from 10 iterations to 5
                    try await Task.sleep(nanoseconds: 50_000_000) // Faster animation (50ms instead of 100ms)
                    await MainActor.run {
                        withAnimation(.spring(response: 0.2, dampingFraction: 0.7)) { // Faster spring animation
                            importProgress = Double(i) / 25.0 // First 20% of progress, faster increment
                        }
                        
                        if i == 3 {
                            withAnimation(.easeInOut(duration: 0.2)) { // Faster text transition
                ...
                // Set status for first major operation
                await MainActor.run {
                    withAnimation(.easeInOut(duration: 0.2)) { // Faster text transition
                ...
                // First major operation - recalculate past scores (60% of progress)
                // This is a significant operation, so we give it more weight
                let startProgress = importProgress
                ...
                let expectedDuration = 1.0 // Expected duration in seconds (reduced from 3.0)
                let updateInterval = 0.05 // Progress update frequency in seconds (reduced from 0.1)
                var elapsed = 0.0
                
                while !pastScoreTask.isCancelled {
                    // Sleep briefly
                    try await Task.sleep(nanoseconds: UInt64(updateInterval * 1_000_000_000))
                    elapsed += updateInterval
                    
                    // Check if the task has completed using the current task
                    if Task.isCancelled {
                        break
                    }
                    
                    // Calculate simulated progress based on elapsed time vs expected duration
                    let progressEstimate = min(elapsed / expectedDuration, 1.0)
                    
                    // Check if we've reached 100% of our simulated progress
                    ...
                    await MainActor.run {
                        withAnimation(.spring(response: 0.2, dampingFraction: 0.7)) { // Faster spring animation
                            importProgress = startProgress + (progressEstimate * importWeight)
                        }
                        
                        // Update status text during the process
                        if elapsed > 0.3 && importStatus != "Analyzing sleep patterns..." { // Reduced from 1.0
                            withAnimation(.easeInOut(duration: 0.2)) { // Faster text transition
                                importStatus = "Analyzing sleep patterns..."
                            }
                        } else if elapsed > 0.6 && importStatus != "Processing baseline metrics..." { // Reduced from 2.0
                            withAnimation(.easeInOut(duration: 0.2)) { // Faster text transition
                ...
                for i in 1...3 { // Reduced from 5 iterations to 3
                    ...
                    await MainActor.run {
                        withAnimation(.spring(response: 0.2, dampingFraction: 0.7)) { // Faster spring animation
                            importProgress = 0.8 + (Double(i) / 15.0) // Final 20% (from 80% to 100%), faster increment
                        }
                ...
                // Complete the import
                await MainActor.run {
                    withAnimation(.spring(response: 0.2, dampingFraction: 0.8)) { // Faster spring animation
                        importProgress = 1.0
                    }
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) { // Reduced from 0.5
                        ...
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) { // Reduced from 0.3
                            withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) { // Faster spring animation
                                animateIcons = true
                                animateContent = true
                            }
                        }
...
struct MockParticlesView: View {
    ...
    init(readinessScore: Double, categoryColor: Color, showScore: Bool = true) {
        ...
        self._displayScore = State(initialValue: readinessScore)
    }
